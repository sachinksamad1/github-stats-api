...
export function generateSVG(
  ...
  const languageList = stats.topLanguages
    .map(
      ([lang, count], i) =>
        `<text x="240" y="${100 + i * 20}" class="text">${lang} (${count} repos)</text>`
    )
    .join("");
  ...
  <text x="35" y="255" class="text-bold">${stats.mostStarredRepo.name}</text>
  <text x="35" y="270" class="text-small">${(stats.mostStarredRepo.description || "No description").substring(0, 60)}${(stats.mostStarredRepo.description?.length || 0) > 60 ? "..." : ""}</text>
...
<svg width="420" height="300" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="20" y="35" class="title">${stats.name}'s Stats</text>
  <text x="20" y="55" class="text-small">@${username} • Joined ${stats.joined}</text>
...
<svg width="300" height="120" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="15" y="25" class="title">${stats.name}'s Compact Stats</text>
  ...
  <text x="215" y="75" class="text-bold">${stats.topLanguages[0]?.[0] || "N/A"}</text>
</svg>
`;
}
